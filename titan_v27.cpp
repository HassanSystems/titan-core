/*
 * TITAN V27 - DAY 62: SQLITE DATABASE MIGRATION
 * * ARCHITECTURE UPGRADES FOR FUTURE DEVELOPERS:
 * 1. SQLITE MIGRATION: Ripped out brittle .txt file memory. Memory is now 
 * securely stored in a relational 'titan.db' SQLite database.
 * 2. PARAMETERIZED QUERIES: append_memory() uses sqlite3_bind_text to 
 * prevent SQL injection and properly handle special characters.
 * 3. FAST RETRIEVAL: load_recent_memory() queries the last 50 rows instantly, 
 * bypassing the need to load the entire history into RAM.
 */
...
string build_system_prompt() {
    return R"(You are Titan V27, an autonomous AI agent integrated into a multiplayer chat network.
...
int main() {
    ...
    const char* sql_create = "CREATE TABLE IF NOT EXISTS Memory (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, entry TEXT);";
    sqlite3_exec(db_connection, sql_create, 0, 0, 0);
    ...
    cout << "\n=== TITAN V27 NETWORK AGENT ONLINE ===\n";
    cout << ">> Architecture: SQLite Engine & Spam Firewall\n";
