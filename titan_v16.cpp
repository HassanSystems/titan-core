...
void read_file_content(string relative_path) {
    ...
    if (f.is_open()) {
        ...
        // Prevent memory poisoning by summarizing instead of dumping
...
void click_mouse_safe(int x, int y) {
    ...
    if (x < 0 || x > screen_w || y < 0 || y > screen_h) {
        cout << ">> [SAFETY]: Click blocked." << endl;
        return;
    }
    SetCursorPos(x, y);
    Sleep(50);
...
void execute_single_action(string action_line) {
    ...
    cout << ">> [EXECUTOR]: Attempting " << action_line << endl;
    ...
    if (action_line.rfind("LIST:", 0) == 0) {
        ...
        cout << ">> [EXECUTOR]: Unknown command: " << action_line << endl;
    }
}
...
int main() {
    ...
    cout << ">> [TITAN V16] ARCHITECTURE ONLINE." << endl;
    cout << ">> [SECURITY]: Strict Plan/Act Separation Enabled." << endl;
    speak("Titan V16 Online.");
    ...
    while (true) {
        ...
        string strict_prompt = R"(You are Titan, a deterministic execution assistant.

You exist ONLY to:
- analyze the user request
- choose ONE valid command from the allowed list
- or choose NONE

ABSOLUTE RULES:
1. Do NOT invent context or simulate stories.
2. Do NOT fabricate file contents.
3. If information is missing, choose ACTION: NONE.

RESPONSE FORMAT (MANDATORY):

PLAN:
- One short sentence explaining what you intend to do.

ACTION:
- EXACTLY ONE of the following, or NONE.
...
- You are sandboxed inside C:\TitanWorkspace
- You cannot escape the workspace

If the user request is unsafe or impossible:
ACTION: NONE)";
        ...
        if (res && res->status == 200) {
            try {
                ...
                if (action_pos != string::npos) {
                    string segment = answer.substr(action_pos + 7);
                    stringstream ss(segment);
                    string potential_cmd;
                    getline(ss, potential_cmd);

                    // Trim whitespace
                    potential_cmd.erase(0, potential_cmd.find_first_not_of(" \n\r\t"));
                    potential_cmd.erase(potential_cmd.find_last_not_of(" \n\r\t") + 1);

                    if (!potential_cmd.empty()) action = potential_cmd;
                }
