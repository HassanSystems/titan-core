/*
 `
 * TITAN V25 - DAY 60: CONTEXT BOUNDING & SECURITY FIREWALL
 * 
 * * ARCHITECTURE UPGRADES FOR FUTURE DEVELOPERS:
 * 1. INGESTION TRUNCATION: append_memory() slices inputs exceeding MAX_INPUT_CHARS 
 * (500) to prevent prompt poisoning and memory bloat.
 * 2. DYNAMIC CONTEXT LOADER: load_recent_memory() pulls exact byte sizes 
 * (MAX_MEMORY_CHARS = 4000) reading backward, replacing brittle line counts.
 * 3. TOKEN BUDGETING: API payload strictly bounds num_ctx (4096) and num_predict (1024) 
 * to guarantee context fits safely within system RAM constraints.
 * 4. PRE-EXECUTION FIREWALL: validate_action() rejects empty targets, and 
 * is_valid_result() sanitizes leaked JSON/tool tags before network transmission.
...
void append_memory(const string &entry) {
    ...
    if (safe_entry.length() > MAX_INPUT_CHARS) {
        safe_entry = safe_entry.substr(0, MAX_INPUT_CHARS) + "... [TRUNCATED FOR CONTEXT LIMITS]";
    }
    ofstream f(MEMORY_FILE, ios::app);
    if (f.is_open()) f << safe_entry << endl;
}
...
string load_recent_memory() {
    ...
    for (auto it = lines.rbegin(); it != lines.rend(); ++it) {
        if (current_chars + it->length() > MAX_MEMORY_CHARS) break;
        selected_lines.push_back(*it);
        current_chars += it->length() + 1; 
    }

    reverse(selected_lines.begin(), selected_lines.end());
    for (const auto& sl : selected_lines) result += sl + "\n";
    return result;
}
...
string build_system_prompt() {
    return R"(You are Titan V25, an autonomous AI agent integrated into a multiplayer chat network.
...
int main() {
    ...
    cout << "\n=== TITAN V25 NETWORK AGENT ONLINE ===\n";
    cout << ">> Architecture: Context Bounding & JSON Sandbox\n";
