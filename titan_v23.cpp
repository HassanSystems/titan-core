// TITAN CORE - V23 (Asynchronous Network Node, Hardened ReAct, qwen2.5-coder:7b)
...
string build_system_prompt() {
    return R"(You are Titan V23, an autonomous AI agent connected to a chat network.
...
4. SEARCH QUERIES MUST BE SIMPLE. Do NOT use quotation marks. Do NOT mention the search engine.
   - BAD: SEARCH:DuckDuckGo "price of gold"
   - GOOD: SEARCH:price of gold today
...
7. If you are unsure about a destructive action, verify with the user first.
8. Once you have finished your task, output your final answer to the user starting with RESULT:

FORMAT:
PLAN: [Reasoning for what to do next]
    ...
    while (titan_running) {
        ...
        while (true) {
            ...
            if (msg.type == "PRIVATE" && msg.to == "titan") {
                cout << "\n>> [NETWORK] Received command from @" << msg.from << ": " << msg.body << endl;
...
int main() {
    ...
    cout << "\n=== TITAN V23 NETWORK AGENT ONLINE ===\n";
