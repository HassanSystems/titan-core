...
//
// ARCHITECTURE:
// This agent operates on a continuous "Plan-Act-Verify" loop.
// 1. INPUT: Receives natural language from user.
// 2. PLAN: Generates a sequence of actions (WRITE, CMD, READ).
// 3. EXECUTE: Runs actions and captures standard output + error streams (`_popen`).
// 4. VERIFY: If a command returns a non-zero exit code:
//    - The error output is fed back into the LLM context.
//    - The agent enters an "Auto-Fix" state to rewrite the failing code.
//
// SECURITY:
// - Sandbox: All file operations restricted to WORKSPACE_ROOT.
// - Human-in-the-Loop: Hazardous actions (CMD, WRITE) require explicit 'y/n' confirmation.
//
// DEPENDENCIES:
// - Ollama (Local LLM API running on port 11434)
// - Winsock2 (Networking)
...
// ==================================================================================
#define _CRT_SECURE_NO_WARNINGS // invalidates specific visual studio warnings

// Winsock must be included before Windows.h to prevent redefinition errors
#include <winsock2.h>
#include <windows.h>
...
// Command execution (uses _popen to capture output)
...
void generate_hearing_script() {
    // Generates a temp python script for audio recording
...
struct Action {
    string type;    // WRITE, CMD, LIST, READ, TYPE, CLICK, WATCH, SYS, SPEAK
    string target;  
    string content; // used for WRITE
};
        ...
        // Generic single-line actions
            ...
            if (!type_str.empty()) {
                ...
                if (type_str == "WATCH") {
                    ...
                    // +1 if no colon in label string above, but they all mostly have colons or logic handles it
                    // Fix offset logic manually for simplicity:
...
string execute_action(const Action &act) {
    if (act.type == "WRITE") {
        ...
        append_memory("OBSERVATION (Vision): " + s);
        return "Saw: " + s;
...
int main() {
    ...
    cout << "\n=== TITAN V18 CREATOR MODE ===\n";
    cout << ">> Workspace: " << WORKSPACE_ROOT << endl;
    speak("Titan V18 Online.");
    ...
    while (true) {
        ...
        for (size_t i = 0; i < actions.size(); i++) {
            ...
            if (!actions[i].target.empty()) cout << " -> " << actions[i].target;
            cout << endl;

            string result = execute_action(actions[i]);

            if (actions[i].type == "CMD") {
                if (result.find("EXIT:0") == string::npos && result.find("CANCELLED") == string::npos) {
                    had_error = true;
        ...
        while (had_error && retry < MAX_RETRY_ATTEMPTS) {
            ...
            for (const auto &act : fix_actions) {
                cout << "\n>> Fix Action: " << act.type << " -> " << act.target << endl;
                string result = execute_action(act);
                if (act.type == "CMD" && result.find("EXIT:0") == string::npos) {
                    had_error = true;
                    last_cmd_output = result;
                    break;
                }
