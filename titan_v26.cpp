/*
 * TITAN V26 - DAY 61: COMMAND RATE LIMITING & SPAM FIREWALL
 * * ARCHITECTURE UPGRADES FOR FUTURE DEVELOPERS:
 * 1. RATE LIMITING: Implemented per-user command cooldowns (15s) using an 
 * unordered_map to track timestamps.
 * 2. EARLY REJECTION: Bypasses the LLM queue entirely for spam requests, firing
 * back a TCP rejection payload to save CPU cycles and prevent engine lockup.
 * (Previous upgrades: Context Bounding, Dynamic Context Loader, Memory Truncation)
 */
...
string build_system_prompt() {
    return R"(You are Titan V26, an autonomous AI agent integrated into a multiplayer chat network.
...
int main() {
    ...
    cout << "\n=== TITAN V26 NETWORK AGENT ONLINE ===\n";
    cout << ">> Architecture: Context Bounding & Spam Firewall\n";
